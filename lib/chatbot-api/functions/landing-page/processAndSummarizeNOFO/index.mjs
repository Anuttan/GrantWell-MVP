...
/**
 * Retry helper function with exponential backoff for Textract rate limit errors
 */
async function retryWithBackoff(operation, maxRetries = 5, baseDelay = 1000) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      // Check if it's a ProvisionedThroughputExceededException
      const isRateLimitError = 
        error.name === 'ProvisionedThroughputExceededException' ||
        error.Code === 'ProvisionedThroughputExceededException' ||
        (error.message && error.message.includes('Provisioned rate exceeded'));

      if (isRateLimitError && attempt < maxRetries - 1) {
        // Exponential backoff with jitter: baseDelay * 2^attempt + random(0-1000ms)
        const delay = baseDelay * Math.pow(2, attempt) + Math.random() * 1000;
        console.warn(
          `Textract rate limit exceeded (attempt ${attempt + 1}/${maxRetries}). Retrying in ${Math.round(delay)}ms...`
        );
...
/**
 * Sleep function with random jitter to spread out requests
 */
function sleep(ms) {
  ...
  for (const record of event.Records) {
    try {
      ...
      if (documentKey.split("/").pop() == "NOFO-File-PDF") {
        // Add random delay (0-2 seconds) to spread out Textract requests and avoid rate limits
        const initialDelay = Math.random() * 2000;
        await sleep(initialDelay);

        // Use AWS Textract for PDF content extraction
        // Start Textract text detection on the PDF with retry logic
        const startParams = {
          DocumentLocation: {
            S3Object: {
              Bucket: s3Bucket,
        ...
        const startResponse = await retryWithBackoff(async () => {
          return await textractClient.send(startCommand);
        });
        ...
        while (textractStatus === "IN_PROGRESS") {
          ...
          // Use retry logic for status polling as well
          const jobStatusResponse = await retryWithBackoff(async () => {
            return await textractClient.send(getJobStatusCommand);
          });
        ...
        if (textractStatus === "SUCCEEDED") {
          ...
          do {
            ...
            // Use retry logic for getting results as well
            const response = await retryWithBackoff(async () => {
              return await textractClient.send(getResultCommand);
            });
